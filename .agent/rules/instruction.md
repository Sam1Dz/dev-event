...
## Project Context
- **Stack**: Next.js 16 (App Router), React 19, Bun, Tailwind CSS v4, HeroUI v2, MongoDB (Mongoose).
...
- **API Handlers**:
  ...
  - Return standardized responses via `apiSuccess` / `apiError` helpers (`src/backend/libs/response.ts`).
- **Models**: Mongoose schemas in `src/backend/models/`.

## Gotchas
- **Hydration**: `suppressHydrationWarning` is required on `html` tag.
- **Server Components**: Root layout is Server; wrap children in `Providers` (Client).
